...
namespace Cyrillic.Convert
{
    public class Conversion
    {
        ...
        private string ConvertCyrillicToLatin(Language language, string text)
        {
            ...
            // Multi-char (Cyrillic->Latin) ordered replacement (longest first) for determinism
        ...
        private string ConvertLatinToCyrillic(Language language, string text)
        {
            ...
            // Multi-char (Latin->Cyrillic) derived from special dictionary values -> keys
            var specialPairs = letters.GetSpecialDictionary();
            if (specialPairs.Count > 0)
            {
                // Order by descending length of Latin sequence (Value) to avoid partial replacements
            ...
            // Replace start sequences if any
            ...
            // Ordered Latin->Cyrillic replacement (longest keys first) to avoid partial consumption (e.g. Eh -> Է + h)
            ...
            if (toCyr.Count > 0)
            {
                ...
                {
                    text = text.Replace(pair.Key, pair.Value);
                    // For single-letter keys only, attempt simple uppercase variant
